...
<div class="card border-success mb-3" style="max-width: 50rem; text-align:left;margin:60px; margin-bottom:70px;">
...
<fieldset>
   <div class="form-group">
    	...
    	보낼 학생 ID</label>
    	<input type="text" name="receiver" class="form-control" placeholder="제목을 입력하세요" id="inputDefault">
    </div>
    <div class="form-group">
    	<label class="col-form-label" for ="inputDefault" style="font-size:20px;font-weight:bold;">
    	제목</label>
  ...
  <center><button type="submit" class="btn btn-success" align="center" style="font-weight:bold;width:60px;">전송</button></center>
</form>
<%
...
<tr class="table-success" style="text-align:center;">
	<th scope="row">읽음</th>
   <th scope="row">보낸학생</th>
   <th scope="row">보낸학생ID</th>
   <th scope="row">시간</th>
   <th scope="row">제목</th>
   </tr>
   <%
   mySQL = "SELECT * FROM MESSAGE WHERE M_RECEIVER = "+session_id;
   ...
   if (myResultSet != null) { 
      while (myResultSet.next()) { 
         ...
         String mySQL2 = "SELECT S_NAME FROM STUDENT WHERE S_ID = "+m_sender;
         myResultSet2 = stmt2.executeQuery(mySQL2);
         String out_p_id="";
         if(myResultSet2!=null){
            while (myResultSet2.next()){
               out_p_id = myResultSet2.getString("s_name");
            }
         }
   ...
   <script>
      ...
      function func(sender,time,title,text,p_id){
    	  window.open("p_popup_message.jsp?m_sender="+sender+"&m_sendtime="+time+"&m_title="+title+"&m_text="+text+"&p_id="+p_id,"a","width=500, height=250, left=500, top=50");
      }
